...
export default function Sidebar() {
  ...
  return (
    <aside className="w-[280px] bg-slate-950 h-screen flex flex-col fixed left-0 top-0 z-40 border-r border-slate-800/50 overflow-y-auto">
      ...
      <nav className="flex-1 px-4 space-y-1 mb-6">
        ...
        {NAV.map((item) => (
          <NavLink
            ...
            className={({ isActive }) => `
              group flex items-center gap-4 px-4 py-3 rounded-[1.25rem] transition-all duration-300
              ${isActive 
            ...
            {({ isActive }) => (
              <>
                <div className={`
                  ...
                  ${isActive ? "bg-white/10" : "bg-slate-900 text-slate-400 group-hover:text-white"}
                `}>
